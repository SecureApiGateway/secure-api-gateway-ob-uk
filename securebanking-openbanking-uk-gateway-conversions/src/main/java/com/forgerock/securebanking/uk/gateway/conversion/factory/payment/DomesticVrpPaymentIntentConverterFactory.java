...
public class DomesticVrpPaymentIntentConverterFactory {
    private static final Logger logger = LoggerFactory.getLogger(DomesticVrpPaymentIntentConverterFactory.class);

    public static GenericIntentConverter getConverter(OBVersion version) {
        switch (version) {
            case v3_1_8: // OBDomesticVRPConsentResponse
            case v3_1_9:
            case v3_1_10:
                return new DomesticStandingOrderIntentConverter6();
            default: {
                String message = String.format("Couldn't find the %s converter for version %s", IntentType.DOMESTIC_VRP_PAYMENT_CONSENT.name(), version.name());
